...
public class HrcLyricsFileReader extends LyricsFileReader {
	...
	@Override
	public LyricsInfo readLrcText(String dynamicContent, String lrcContent,
			String extraLrcContent, String lyricsFilePath) throws Exception {
		return null;
	}
	...
	public LyricsInfo readInputStream(InputStream in) throws Exception {
		...
		if (in != null) {
			...
			for (int i = 0; i < lyricsTexts.length; i++) {
				...
				// 解析歌词
				parserLineInfos(lyricsIfno, lyricsLineInfosTemp, lyricsTags,
						lyricsTexts[i]);
	...
	private void parserLineInfos(LyricsInfo lyricsIfno,
		...
		if (lineInfo.startsWith(LEGAL_TITLE_PREFIX)) {
			...
			// 解析翻译歌词
			// 获取json base64字符串
			String translateJsonBase64String = lineInfo.substring(
			...
			String[] lineComments = lineInfo.substring(leftIndex + 1,
					rightIndex).split("'\\s*,\\s*'", -1);
			// 歌词
			...
			wordsDisIntervalText = wordsDisIntervalText.substring(
					wordsDisIntervalLeft + 1, wordsDisIntervalRight);
	...
	private void parserOtherLrc(LyricsInfo lyricsIfno,
		...
		for (int i = 0; i < contentArrayObj.size(); i++) {
			...
			if (type == 1) {
				// 解析翻译歌词
				if (lyricsIfno.getTranslateLrcLineInfos() == null
						|| lyricsIfno.getTranslateLrcLineInfos().size() == 0)
					parserTranslateLrc(lyricsIfno, lyricContentArrayObj);

			} else if (type == 0) {
				// 解析音译歌词
				if (lyricsIfno.getTransliterationLrcLineInfos() == null
						|| lyricsIfno.getTransliterationLrcLineInfos().size() == 0)
	 ...
	 * @return
	 */
	private int[] getWordsDisIntervalString(String wordsDisIntervalString)
			throws Exception {
