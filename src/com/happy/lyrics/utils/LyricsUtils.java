...
public class LyricsUtils {
	 ...
	 * @return
	 */
	public int getLineNumber(TreeMap<Integer, LyricsLineInfo> lyricsLineTreeMap, int oldPlayingTime, int offsetTime) {

		// 添加歌词增量
		int curPlayingTime = oldPlayingTime + offsetTime;
		...
		for (int i = 0; i < lyricsLineTreeMap.size(); i++) {
			...
			if (curPlayingTime > lyricsLineTreeMap.get(i).getEndTime() && i + 1 < lyricsLineTreeMap.size()
					&& curPlayingTime < lyricsLineTreeMap.get(i + 1).getStartTime()) {
				return i;
			}
		}
		if (curPlayingTime >= lyricsLineTreeMap.get(lyricsLineTreeMap.size() - 1).getEndTime()) {
	 ...
	 * @return
	 */
	public int getLineNumber(List<LyricsLineInfo> lyricsLineInfos, int oldPlayingTime, int offsetTime) {
		...
		for (int i = 0; i < lyricsLineInfos.size(); i++) {
			...
			if (curPlayingTime > lyricsLineInfos.get(i).getEndTime() && i + 1 < lyricsLineInfos.size()
					&& curPlayingTime < lyricsLineInfos.get(i + 1).getStartTime()) {
				return i;
			}
		}
		if (curPlayingTime >= lyricsLineInfos.get(lyricsLineInfos.size() - 1).getEndTime()) {
	 ...
	 * @return
	 */
	public int getDisWordsIndex(TreeMap<Integer, LyricsLineInfo> lyricsLineTreeMap, int lyricsLineNum,
			int oldPlayingTime, int offsetTime) {
		if (lyricsLineNum == -1)
			return -1;
	 ...
	 * @return
	 */
	public int getDisWordsIndex(List<LyricsLineInfo> lyricsLineInfos, int lyricsLineNum, int oldPlayingTime,
			int offsetTime) {
		if (lyricsLineNum == -1)
			return -1;
	 ...
	 * @return
	 */
	public int getDisWordsIndexLenTime(TreeMap<Integer, LyricsLineInfo> lyricsLineTreeMap, int lyricsLineNum,
			int oldPlayingTime, int offsetTime) {
		if (lyricsLineNum == -1)
			return 0;
		// 添加歌词增量
	 ...
	 * @return
	 */
	public int getDisWordsIndexLenTime(List<LyricsLineInfo> lyricsLineInfos, int lyricsLineNum, int oldPlayingTime,
			int offsetTime) {
		if (lyricsLineNum == -1)
			return 0;
		// 添加歌词增量
	 ...
	 *            歌词文本的宽度
	 */
	public static int[] getLineIndexAndItemLineIndex(int currTime, TreeMap<Integer, LyricsLineInfo> lyricsLineInfos,
			int viewWidth, int textWidth) {
		...
		if (lrcIndex != -1) {
			...
			TreeMap<Integer, LyricsLineInfo> itemLyricsLineInfos = getItemLyricsLineInfo(lyricsLineInfo, viewWidth,
					textWidth);
	 ...
	 * @return
	 */
	private static int getLyricsLineIndex(TreeMap<Integer, LyricsLineInfo> lyricsLineInfos, int currTime) {
		if (currTime >= lyricsLineInfos.get(lyricsLineInfos.size() - 1).getEndTime()) {
			return lyricsLineInfos.size() - 1;
		} else {
			for (int i = 0; i < lyricsLineInfos.size(); i++) {
				...
				if (currTime > lyricsLineInfos.get(i).getEndTime() && i + 1 < lyricsLineInfos.size()
	 ...
	 * @return
	 */
	public static TreeMap<Integer, LyricsLineInfo> getItemLyricsLineInfo(LyricsLineInfo lyricsLineInfo, int viewWidth,
			int textWidth) {
		...
		if (lyricsTextWidth > maxLineWidth) {
			...
			// 最大的歌词行数
			int maxLrcLineNum = lyricsTextWidth % maxLineWidth == 0 ? lyricsTextWidth / maxLineWidth
			...
			for (int i = lyricsWords.length - 1; i >= 0; i--) {
				...
				if (lyricsWordsWidth > maxLrcLineWidth) {
					...
					//
					LyricsLineInfo itemLyricsLineInfo = getItemLyricsLineInfo(wordsDisInterval, lyricsWords, i,
							lastIndex);
					//
					lastIndex = i;
					i++;// 超出的视图宽度，后退一个索引
					...
					//
					LyricsLineInfo itemLyricsLineInfo = getItemLyricsLineInfo(wordsDisInterval, lyricsWords, 0,
							lastIndex);
					if (itemLyricsLineInfo != null)
						itemLyricsLineInfos.put(index++, itemLyricsLineInfo);
				}
	 ...
	 * @return
	 */
	private static LyricsLineInfo getItemLyricsLineInfo(int[] wordsDisInterval, String[] lyricsWords, int startIndex,
			int endIndex) {

		if (endIndex < 0 || startIndex < 0 || startIndex >= lyricsWords.length || endIndex >= lyricsWords.length
				|| startIndex > endIndex)
			return null;
